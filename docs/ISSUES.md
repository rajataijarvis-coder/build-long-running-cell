...
**Status:** Fixed in commits `ac74b0e` (chapters 21 and 22) and `7b7e58b` (chapters 04–20). Chapter 23 already contained the note. All affected chapters now include a "files already exist in repo" note at the top.
...
**Status:** Fixed in commit `d762ec7`. Added a "Post-publication updates" section to `docs/CHAPTER_CROSS_REFERENCE.md` mapping `factory.ts`, `main-dark.ts`, `ServerContext`, `accountability-store.ts`, and `docs/ACCOUNTABILITY.md` to their reference docs.
...
**Fix:**
- [x] Compare each diagram against the current source code.
- [x] Fix any mismatches (e.g., renamed files, missing methods).
- [x] Add a note at the top: "Last verified against commit ...".

**Status:** Fixed in commit `50d23a1`. Added verification note, `ServerContext`, `AccountabilityStore`, `getPlanner()`, `buildAccountabilityContract`, `listAccountability`, `FailureMemory`, shared services, and updated method signatures.
...
**Fix:**
- [x] Add `cell/src/factory.test.ts` that asserts `createDarkFactoryContext` returns a context whose `hitl` approves every action, whose `guardrails` still block prompt-injection / path-escape / network-egress, and whose `verificationCommands` and `budget` are wired into the cell. (commit `7fd53e1`)

**Status:** Resolved.

**Files:** `cell/src/factory.test.ts` (new), `cell/src/cell.ts`
