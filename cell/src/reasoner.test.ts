...
describe('Reasoner', () => {
  it('picks the first plan step on the first call', async () => {
    ...
    const thought = await reasoner.reason(makePlan('verify'), undefined, undefined, 'verify the code');
  ...
  it('advances to the next step after a successful observation', async () => {
    ...
    const first = await reasoner.reason(plan, undefined, undefined, 'verify');
    const observation: Observation = {
      stepId: first.stepId,
      output: 'all green',
      success: true,
    };
    const second = await reasoner.reason(plan, first, observation, 'verify');
  ...
  it('retries the same step after a failed observation', async () => {
    ...
    const retry = await reasoner.reason(plan, first, observation, 'verify');
    assert.equal(retry.stepId, 's1');
    assert.match(retry.action.input, /retry after/);
  });

  it('skips a step that has already succeeded', async () => {
    const reasoner = new Reasoner();
    const plan = makePlan('verify');
    const first = await reasoner.reason(plan, undefined, undefined, 'verify');
    ...
    const retry = await reasoner.reason(plan, first, okObservation, 'verify');
